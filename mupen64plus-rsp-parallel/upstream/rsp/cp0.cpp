...
extern "C"
{
	 ...
	 * (row 12), covering wrapped regions. This arm derives its geometry
	 * from the unmodified length register and never touches the legacy path.
	 */
	...
	static int rsp_dma_read(RSP::CPUState *rsp)
	{
		...
		/*
		 * The explicit DD runtime policy independently selects corrected
		 * transfer semantics. All other sessions retain the legacy path.
		 */
		if (RSP::DdRuntimePolicyEnabled() != 0)
			return rsp_dma_read_corrected(rsp, length_reg);
